...
export class WebviewPanel {
  ...
  constructor(
    ...
    _log = vscode.window.createOutputChannel('VibeTracker Dashboard');
  }
  ...
  private buildHtml(): string {
    ...
    _log.appendLine(`[buildHtml] threshold=${threshold} interval=${interval} template=${template}`);
    ...
    // use command: URIs which are more reliable than postMessage
    const cmdUrl = (key: string, val: any) =>
      'command:vibetracker._setConfig?' + encodeURIComponent(JSON.stringify({key, val}));
  ...
  <div style="background:#161b22;border:1px solid #30363d;border-radius:8px;padding:16px;margin-bottom:16px">
    <div style="margin-bottom:14px">
      ...
      <div style="display:flex;align-items:center;gap:8px;margin-top:4px">
        <a href="${cmdUrl('savesThreshold', Math.max(1,threshold-1))}" style="background:#30363d;color:#c9d1d9;border:none;border-radius:4px;padding:4px 10px;cursor:pointer;font-size:14px;text-decoration:none">−</a>
        <span style="font-size:18px;font-weight:700;color:#58a6ff;min-width:30px;text-align:center">${threshold}</span>
        <a href="${cmdUrl('savesThreshold', threshold+1)}" style="background:#30363d;color:#c9d1d9;border:none;border-radius:4px;padding:4px 10px;cursor:pointer;font-size:14px;text-decoration:none">+</a>
      </div>
    </div>
    <div style="margin-bottom:14px">
      <label style="font-size:12px;color:#8b949e">Idle flush interval (minutes)</label>
      <div style="display:flex;align-items:center;gap:8px;margin-top:4px">
        <a href="${cmdUrl('flushInterval', Math.max(1,interval-5))}" style="background:#30363d;color:#c9d1d9;border:none;border-radius:4px;padding:4px 10px;cursor:pointer;font-size:14px;text-decoration:none">−</a>
        <span style="font-size:18px;font-weight:700;color:#58a6ff;min-width:30px;text-align:center">${interval}</span>
        <a href="${cmdUrl('flushInterval', interval+5)}" style="background:#30363d;color:#c9d1d9;border:none;border-radius:4px;padding:4px 10px;cursor:pointer;font-size:14px;text-decoration:none">+</a>
      </div>
    </div>
    <div>
      <label style="font-size:12px;color:#8b949e;display:block;margin-bottom:4px">SVG Template</label>
      <select onchange="window.location.href='${cmdUrl('template', "'+this.value+'")}'" style="background:#0d1117;color:#c9d1d9;border:1px solid #30363d;border-radius:4px;padding:6px 8px;font-size:12px;width:100%">
        ${tmplOpts}
      </select>
    </div>
  </div>
